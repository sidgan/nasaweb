  ...
  return (
    <section>
      ...
      <div className="col-lg-7">
        ...
        >
          {showTooltip ? (
            <Tooltip title={props.dataPoint} TransitionComponent={Zoom}>
              <p>He</p>
            </Tooltip>
          ) : null}
        </GlobeObject>
...
const GlobeObject = ({ date, showTooltip }) => {
  ...
  Promise.all(promises).then(() => {
    ...
    let rMeteorScale = d3
      .scaleLinear()
      .domain(
        d3.extent(meteorJson.features, function (d) {
    ...
    let rStarScale = d3
      .scaleLinear()
      .domain(
        d3.extent(starJson.features, function (d) {
          return d.properties.mag;
        })
      )
      .range([2.5, 1]);
    ...
    let rSunScale = d3
      .scaleLinear()
      .domain(
        d3.extent(sunJson.features, function (d) {
          return d.properties.mag;
        })
      )
      .range([4, 6]);
    ...
    let handleMouseClick = function (d) {
      window.open(
        'https://www.meteorshowers.org/view/iau-' + d.properties.name
      );
    };

    // Place data points
    // METEOR
    let meteorsDataPoint = svg
    ...
    // SUN
    let sunsDataPoint = svg
      .selectAll('path.sun')
      .data(sunJson.features)
      .enter()
      .append('path')
      .attr('class', 'sun')
      .attr('d', sunPath)
    ...
    let dragBehavior = d3.drag().subject(Object).on('drag', drag);
